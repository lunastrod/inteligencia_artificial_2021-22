...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for i in range(len(foodGrid.asList())):
        ...
        for f in foodGrid.asList():
            distances.append(util.manhattanDistance(f,position))
            
        #print(distances)
        #print(foodGrid.asList()[distances.index(min(distances))])

        total_distance+=min(distances)
        position=foodGrid.asList()[distances.index(min(distances))]
        pop()
