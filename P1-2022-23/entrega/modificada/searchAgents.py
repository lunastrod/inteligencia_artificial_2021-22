...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state: Any):
        ...
        if pos in self.corners:
            ...
            return len(Visited_Corners) == 4 #devolvemos true si hemos visitado todos los corners
        else:
            return False 
            ...
            if not hitsWall:
                ...
                if next_node in self.corners:
                    if next_node not in sucVCorners:
                        sucVCorners.append(next_node) #añadimos el corner a la lista de corners visitados
                successor = ((next_node, sucVCorners), action, 1) 
                successors.append(successor) #añadimos el sucesor a la lista de sucesores
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for i in range(4):
        if corners[i] not in Visited_Corners:
            un_Visited_Corner.append(corners[i]) #añadimos los corners no visitados a la lista de corners no visitados

    #print(un_Visited_Corner)

    cur_position = node
    while(len(un_Visited_Corner)!=0): #mientras queden corners por visitar
        distance, corner = min( [(util.manhattanDistance(cur_position ,corner),corner) for corner in un_Visited_Corner] ) #calculamos la distancia manhattan al corner mas cercano
        h_sum = h_sum + distance
        cur_position = corner
        un_Visited_Corner.remove(corner) #eliminamos el corner mas cercano de la lista de corners no visitados
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    #return len(foodGrid.asList()) #esta heurística es muy curiosa

    food_list = foodGrid.asList()
    distances=[]
    for i in range(len(food_list)): #calculamos la distancia de cada comida a la posición actual
        distances.append(mazeDistance(position, food_list[i], problem.startingGameState))
    if(len(distances)==0):
        return 0
    return max(distances)#devolvemos la máxima distancia de las comidas a la posición actual
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        "*** YOUR CODE HERE ***"
        return (x, y) in self.food.asList() #si la posición actual está en la lista de comidas
