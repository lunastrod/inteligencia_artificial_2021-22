...
def positionLogicPlan(problem):
    ...
    for x in range(1, width+1) :
        for y in range(1, height+1) :
            initial_locations.append(logic.PropSymbolExpr("P", x, y, 0))
    initial = exactlyOne(initial_locations) & logic.PropSymbolExpr("P", x0, y0, 0)
    ...
    for t in range(MAX_TIME_STEP):
        ...
        if t > 0:
            ...
            suc = logic.conjoin(succ)
            if successors:
                success = logic.conjoin(suc, logic.conjoin(successors))
            else:
                success = suc
            ...
            j = findModel(logic.conjoin(initial, goal, exclus, success))
