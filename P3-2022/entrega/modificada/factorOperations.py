...
def joinFactors(factors):
    ...
    #now we have all the variables, we need to remove the unconditioned variables from the conditioned variables
    conditionedVariables = conditionedVariables.difference(unconditionedVariables)
    newFactor = Factor(unconditionedVariables, conditionedVariables, list(factors)[0].variableDomainsDict())
    #variableDomainsDict is the same for all factors, so we can just take the first one
    for assignment in newFactor.getAllPossibleAssignmentDicts():
        product = 1
        for factor in factors:
            product *= factor.getProbability(assignment)
        #now we have the product of all the factors, we need to set the probability of the new factor
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        #to eliminate, we need to sum over the elimination variable
        for assignment in newFactor.getAllPossibleAssignmentDicts():
            result = 0
...
def normalize(factor):
    ...
    #now we have the correct conditioned and unconditioned variables
    ...
    if sum == 0:
        return None #if the sum is 0, we can't normalize
    for assignment in newFactor.getAllPossibleAssignmentDicts():
