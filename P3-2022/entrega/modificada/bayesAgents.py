...
def constructBayesNet(gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):# for each wall in each house
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    highestProb = -100000000000000000000000 #arbitrary low number
    ...
    return highestAssignDict #contains the highest probability and the assignment
    "*** END YOUR CODE HERE ***"
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        CPT = inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence, eliminationOrder) #get the joint distribution

        ftopleft = {} #evidence for the left house
        ftopleft.update(evidence) 
        ftopleft[FOOD_HOUSE_VAR] = TOP_LEFT_VAL 
        ftopleft[GHOST_HOUSE_VAR] = TOP_RIGHT_VAL 

        ftopright = {} #evidence for the right house
        ...
        #expected value of entering the left house = probability of entering the left house * reward of entering the left house + probability of entering the right house * reward of entering the right house
        "*** END YOUR CODE HERE ***"
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        probsandoutcomes = self.getExplorationProbsAndOutcomes(evidence) #get the probability of each outcome and the evidence for each outcome
        ...
        for (prob, evidence0) in probsandoutcomes: 
            ...
            #expected value of exploring the remaining house = probability of each outcome * max value of entering the house
...
def constructInsuranceBayesNet():
    "*** MODIFICATION ***"
    """
    this method is similar to constructBayesNet:
    """
    #create the variables
    variables = [INCOME_VAR, DEPOSIT_VAR, PAYMENT_VAR, HOUSING_VAR, SECURITY_VAR]
    #create the edges
    edges = [(INCOME_VAR, DEPOSIT_VAR), (INCOME_VAR, PAYMENT_VAR), (DEPOSIT_VAR, PAYMENT_VAR), (HOUSING_VAR, SECURITY_VAR), (PAYMENT_VAR, SECURITY_VAR)]
    #create the domains
    variableDomainsDict = {}
    ...
    #fillCPT
    income_factor = bn.Factor([INCOME_VAR], [], net.variableDomainsDict())
    income_factor.setProbability({INCOME_VAR:HIGH_INCOME}, 0.3)
    income_factor.setProbability({INCOME_VAR:HIGH_INCOME}, 0.7)
    net.setCPT(INCOME_VAR, income_factor)

    housing_factor = bn.Factor([HOUSING_VAR], [], net.variableDomainsDict())
    housing_factor.setProbability({HOUSING_VAR:REAL_STATE}, 0.35)
    housing_factor.setProbability({HOUSING_VAR:TENANT}, 0.65)
    net.setCPT(HOUSING_VAR, housing_factor)
    
    deposit_factor = bn.Factor([DEPOSIT_VAR], [INCOME_VAR], net.variableDomainsDict())
    deposit_factor.setProbability({INCOME_VAR:HIGH_INCOME, DEPOSIT_VAR:LARGE_DEPOSIT}, 0.1)
    deposit_factor.setProbability({INCOME_VAR:HIGH_INCOME, DEPOSIT_VAR:SMALL_DEPOSIT}, 0.9)
    deposit_factor.setProbability({INCOME_VAR:LOW_INCOME, DEPOSIT_VAR:LARGE_DEPOSIT}, 0.6)
    deposit_factor.setProbability({INCOME_VAR:LOW_INCOME, DEPOSIT_VAR:SMALL_DEPOSIT}, 0.4)
    net.setCPT(DEPOSIT_VAR, deposit_factor)

    payment_factor = bn.Factor([PAYMENT_VAR],[INCOME_VAR, DEPOSIT_VAR],  net.variableDomainsDict())
    payment_factor.setProbability({INCOME_VAR:HIGH_INCOME, DEPOSIT_VAR:LARGE_DEPOSIT, PAYMENT_VAR:DEFAULT}, 0.05)
    payment_factor.setProbability({INCOME_VAR:HIGH_INCOME, DEPOSIT_VAR:LARGE_DEPOSIT, PAYMENT_VAR:PAY_BACK}, 0.95)
    payment_factor.setProbability({INCOME_VAR:HIGH_INCOME, DEPOSIT_VAR:SMALL_DEPOSIT, PAYMENT_VAR:DEFAULT}, 0.5)
    payment_factor.setProbability({INCOME_VAR:HIGH_INCOME, DEPOSIT_VAR:SMALL_DEPOSIT, PAYMENT_VAR:PAY_BACK}, 0.5)
    payment_factor.setProbability({INCOME_VAR:LOW_INCOME, DEPOSIT_VAR:LARGE_DEPOSIT, PAYMENT_VAR:DEFAULT}, 0.45)
    payment_factor.setProbability({INCOME_VAR:LOW_INCOME, DEPOSIT_VAR:LARGE_DEPOSIT, PAYMENT_VAR:PAY_BACK}, 0.55)
    payment_factor.setProbability({INCOME_VAR:LOW_INCOME, DEPOSIT_VAR:SMALL_DEPOSIT, PAYMENT_VAR:DEFAULT}, 0.6)
    payment_factor.setProbability({INCOME_VAR:LOW_INCOME, DEPOSIT_VAR:SMALL_DEPOSIT, PAYMENT_VAR:PAY_BACK}, 0.4)
    net.setCPT(PAYMENT_VAR, payment_factor)

    security_factor = bn.Factor([SECURITY_VAR], [HOUSING_VAR, PAYMENT_VAR],  net.variableDomainsDict())
    security_factor.setProbability({HOUSING_VAR:REAL_STATE, PAYMENT_VAR:DEFAULT, SECURITY_VAR:SECURITY_GIVEN}, 0.01)
    security_factor.setProbability({HOUSING_VAR:REAL_STATE, PAYMENT_VAR:DEFAULT, SECURITY_VAR:NO_SECURITY}, 0.99)
    security_factor.setProbability({HOUSING_VAR:REAL_STATE, PAYMENT_VAR:PAY_BACK, SECURITY_VAR:SECURITY_GIVEN}, 0.75)
    security_factor.setProbability({HOUSING_VAR:REAL_STATE, PAYMENT_VAR:PAY_BACK, SECURITY_VAR:NO_SECURITY}, 0.25)
    security_factor.setProbability({HOUSING_VAR:TENANT, PAYMENT_VAR:DEFAULT, SECURITY_VAR:SECURITY_GIVEN}, 0.5)
    security_factor.setProbability({HOUSING_VAR:TENANT, PAYMENT_VAR:DEFAULT, SECURITY_VAR:NO_SECURITY}, 0.5)
    security_factor.setProbability({HOUSING_VAR:TENANT, PAYMENT_VAR:PAY_BACK, SECURITY_VAR:SECURITY_GIVEN}, 0.31)
    security_factor.setProbability({HOUSING_VAR:TENANT, PAYMENT_VAR:PAY_BACK, SECURITY_VAR:NO_SECURITY}, 0.69)
    net.setCPT(SECURITY_VAR, security_factor)

    return net

def inferenceOnInsuranceBayesNet(bayes_net):
    "*** MODIFICATION ***" 
    return inference.inferenceByEnumeration(bayes_net, SECURITY_VAR, {INCOME_VAR: HIGH_INCOME, HOUSING_VAR: TENANT})
