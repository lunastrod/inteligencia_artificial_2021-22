...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for eli_var in eliminationOrder: # eliminate all variables in the elimination order
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, eli_var)
            if len(joinedFactor.unconditionedVariables()) == 1:
                continue
                
            eliminate_factor = eliminate(joinedFactor, eli_var)
            currentFactorsList.append(eliminate_factor)
        joined_factor = joinFactors(currentFactorsList)
