...
from logic import E, conjoin, disjoin
...
def sentence3() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    WA1 = logic.PropSymbolExpr("PacmanAlive_1")
    WA0 = logic.PropSymbolExpr("PacmanAlive_0")
    WB0 = logic.PropSymbolExpr("PacmanBorn_0")
    WK0 = logic.PropSymbolExpr("PacmanKilled_0")
...
def findModelCheck() -> Dict[Any, bool]:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #I return a dict with a dummyClass instance as key and True as value
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    if(True in findModel(~premise|conclusion).values()):
        #if the model contains a True, then the premise does not entail the conclusion
        return False
    return True
    "*** END YOUR CODE HERE ***"
...
def atLeastOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return logic.disjoin(literals)          #Indica si uno de los valores de literals es True.
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #print(literals)
    #resuelto con un mapa de karnaugh
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #true if and only if at least one of the possible causes is true for the current state
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    #describes how Pacman can end in various locations on this time step.
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #pacphysics axioms con timesteps adecuados
    ...
    #posicion de pacman en t0
    KB.append(PropSymbolExpr(pacman_str, x0,y0,time=0))
    #acciones de pacman en t0 y t1
    KB.append(PropSymbolExpr(action0,time=0))
    KB.append(PropSymbolExpr(action1,time=1))

    fModel1 = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, x1,y1,time=1))
    fModel2 = findModel(conjoin(KB) & ~PropSymbolExpr(pacman_str, x1,y1,time=1))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range (50):
        ...
        for x,y in non_wall_coords:
            coords.append(PropSymbolExpr(pacman_str,x,y,time=t))
            if t != 0:
                #Llama a pacmanSuccessorAxiomSingle(...) para todas las posibles posiciones de Pacman en non_wall_coords.
                KB.append(pacmanSuccessorAxiomSingle(x,y,t,walls_grid))   

        #pacman no puede estar en dos lugares a la vez
        KB.append(exactlyOne(coords))

        directions = []
        for direction in actions:
            directions.append(PropSymbolExpr(direction, time = t))
        KB.append(exactlyOne(directions)) #pacman solo puede tomar una accion a la vez
        
        fModel = findModel(PropSymbolExpr(pacman_str,xg,yg,time=t) & conjoin(KB))
        
        if fModel:
            modelPlan = extractActionSequence(fModel,actions)#extrae la secuencia de acciones
            if modelPlan:
                return modelPlan #si hay un modelo, devuelve la secuencia de acciones
...
def foodLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # pacman empieza en la posicion inicial
    KB.append(PropSymbolExpr(pacman_str,x0,y0,time=0))


    """
    Inicializa las variables Food[x,y]_t con el código PropSymbolExpr(food_str, x, t,
    time=t), donde cada variable es verdadera si y solo si hay un alimento en (x, y) en el
    momento t.
    """
    for x0,y0 in food:
    ...
    for t in range (50):
        ...
        foodList = []        
        for x,y in food:
            posFood = PropSymbolExpr(food_str,x,y,time=t)
            futureFood = PropSymbolExpr(food_str,x,y,time=t+1)
            posPacman = PropSymbolExpr(pacman_str,x,y,time=t)
            """
            Agrega un axioma del sucesor de comida: ¿Cuál es la relación entre Food[x,y]_t+1 y
            Food[x,y]_t y Pacman[x,y]_t? El axioma del sucesor de alimentos solo debe involucrar
            estas tres variables, para cualquier (x, y) y t dados. Piense en cómo se ve el modelo de
            transición para las variables alimentarias y agregue estas oraciones a su base de
            conocimientos en cada paso de tiempo
            """
            KB.append(((posFood & ~posPacman) % futureFood))
            foodList.append(posFood)

        """
        Cambia la afirmación del objetivo: su oración de afirmación de la meta debe ser verdadera
        si y solo si se ha comido toda la comida. Esto sucede cuando todos los Food[x,y]_t son
        falsos.
        """
        fModel = findModel(~atLeastOne(foodList) & conjoin(KB))
        
        if fModel:
            modelPlan = extractActionSequence(fModel,actions)
            if modelPlan:
                return modelPlan
    "*** END YOUR CODE HERE ***"
